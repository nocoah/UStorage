<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/UZero.Master" AutoEventWireup="true" CodeBehind="RoleList.aspx.cs" Inherits="UZeroConsole.Web.UZero.Sso.RoleList" %>
    ...
    <form runat="server">
        ...
        <div class="content bg-gray-lighter">
            <div class="row items-push">
                <div class="col-sm-7">
                    <h1 class="page-heading">Sso角色列表 <small>角色管理权限</small>
                    </h1>
                </div>
            </div>
        </div>
                        ...
                        <div class="form-group ">
                            <label class="col-xs-pull-1">
                                <% if (Model.Apps != null && Model.Apps.Count > 0)
                                    { %>
                                <select class="form-control" id="ddlApps" style="font-weight:normal">
                                    <% foreach(var app in Model.Apps) { %>
                                    <option <%= app.Id == Model.GetAppId? "selected='selected'": "" %> value="<%= app.Id %>"><%= app.ToString() %></option>
                                    <%} %>
                                </select><%} %> <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalCreateRole"><i class="fa fa-plus"></i>添加角色</button>
                            </label>
                        </div>
                    </div>
                </div>
